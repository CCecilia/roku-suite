...
export default {
    ...
    compileZip() {
        return new Promise((resolve, reject) => {
            ...
            for( let i=0; i<zipPaths.files.length; i++ ) {
                archive.append(fs.createReadStream(`${rootDir}/${zipPaths.files[i]}`), { name: `${baseName}/${zipPaths.files[i]}` });
    ...
    deploy() {
        console.log('deploy');
        return atom.notifications.addInfo(`Deployment to ${this.rokuIP} started`);
  ...
  requestCallback(error, response, body) {
      if ((response !== undefined) && (response.statusCode !== undefined) && (response.statusCode === 200)) {
          if (response.body.indexOf("Identical to previous version -- not replacing.") !== -1) {
              ...
              return atom.notifications.addSuccess(`Deployed to ${this.rokuAddress}`);
          }
      } else {
          atom.notifications.addFatalError(`Failed to deploy to ${this.rokuAddress} see console output for details.`);
          console.log(error);
